...
def car_wear(car: dict) -> float:
    """Return wear percentage for a single car; 0 when no last_service_km reading exists."""
...
def fleet_summary(fleet: list[dict]) -> dict:
    """Summarise the whole fleet: car count, number due, and average wear percentage."""
    total = 0.0
    due = 0
    for car in fleet:
        total += car_wear(car)
